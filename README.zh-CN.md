...
**让高能力模型负责思考与决策，让高效率 Agent 负责执行。**

GPTgrill-Codexwork 的核心目的，是把 **ChatGPT 网页端的高品质推理能力** 与 **本地 Codex 的工程执行能力** 串成一套可持续运作的开发工作流。

在 ChatGPT 网页端，可以依不同产品或 Repo 建立独立 Project，使用当下可用的高能力模型（例如 GPT-5.6 Sol High）进行需求讨论、Grill、架构设计、Spec 撰写与关键决策。这一层负责的是「先把要做什么想清楚」，并优先利用 ChatGPT 网页端既有方案与额度，而不是把所有高阶推理都转成额外的 API token 消耗。

需求定案后，GPTgrill-Codexwork 会把结果转成可执行的 Spec、Phase、Task 与 Dispatch，交给本地 Codex。Codex 端则可以使用更适合大量执行工作的模型与设定，例如在环境支援时使用 Luna Max 搭配 subagents，平行处理程式阅读、实作、测试、除错与验证；遇到需要高阶判断或独立 Review 的节点，再透过 `codex-with-chatgpt` 回到 ChatGPT Controller Chat。

换句话说，这套工具不是要让最强模型包办所有工作，而是刻意做**模型分工**：

```text
ChatGPT Web / GPT-5.6 Sol High
  -> Grill / 需求澄清 / 架构 / Spec / 关键决策 / Review

Git + GPTgrill-Codexwork
  -> 保存工程状态 / 任务调度 / Phase & Task 边界 / 自动交接

Codex / Luna Max + subagents（若环境支援）
  -> 大量程式阅读 / 实作 / 测试 / 修复 / 平行工作

ChatGPT Web
  -> 独立审查 / 阶段性决策 / 下一轮规划
```

这样做的目标，是在维持高品质决策的同时，把大量执行工作交给速度更快、成本更低的 Agent 层，降低不必要的高阶模型 token 消耗，并让一个人能更有效率地同时管理多个专案与 Repo。

> 模型名称、方案与可用额度可能随 OpenAI 产品调整。本专案不假设任何模型「永久免费」或「永久无上限」；核心设计是把高成本推理集中在真正需要的节点，并让执行层尽量使用更高效率的模型与 subagents。
...
/goal phase
  ...
  -> Codex 自动执行
  -> ChatGPT 透过 codex-with-chatgpt 自动审查
  -> Codex 自动修复
  -> 生成 Phase Report
  -> 停下来等你确认下一阶段
```
...
| 元件 | 负责 | 不应该负责 |
|---|---|---|
| ChatGPT Web | 需求、规划、Spec、Review、面向使用者的决策 | 本地 shell 执行 |
| Grill Me Skill | 把“到底要做什么”问清楚 | 实现程式码 |
| GPTgrill-Codexwork Web Skill | 把确认后的需求写成 repo artifact + dispatch | 启动本地程序 |
| GPTgrill-Codexwork Runner | 监听 dispatch、决定下一个 Task、状态机、commit/push、report | 产品方向 |
| Codex | 改档案、跑命令、测试、实现 | 长期 scope 决策 |
| codex-with-chatgpt | Codex ↔ ChatGPT 的 plan/review/fix 回圈 | Phase/Task 排程 |
...
codex-with-chatgpt
  ...
  回圈：PLAN -> EXECUTION -> EXECUTED -> REVIEW -> PLAN/DONE
```
...
# ChatGPT Project 和本地 workspace 怎么系结？

GPTgrill-Codexwork 本身不会把“任意网页对话”系结到“任意本地目录”。
...
如果你的 ChatGPT 网页支援直接上传 Skill，分别上传：
...
这样即使 watcher 每 30 秒一直轮询，也不会因为“服务还活著”就偷偷继续下一阶段。
...
## Acceptance Criteria
- [ ] 合法输出透过 schema validation。
- [ ] 非法输出返回确定性的 validation error。
- [ ] 现有 smoke cases 全部保持透过。
...
GPTgrill-Codexwork 实际呼叫 Codex 的形式等价于：
...
如果本地 workspace 有未提交修改，GPTgrill-Codexwork 预设会拒绝开始 autonomous run，避免把你正在做的工作混进自动执行结果里。
...
- macOS（当前内建 background service helper 使用 `launchd`）
- Node.js >= 20
- Git
- 已安装并登入 Codex CLI
- 目标 workspace 已安装并配置好 `codex-with-chatgpt`

前台手动执行并不强依赖 macOS，但自动常驻 watcher 目前使用的是 macOS `launchd`。
...
在系结对应 workspace 的 ChatGPT Project 里：
