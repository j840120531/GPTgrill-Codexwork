# GPTgrill-Codexwork 发布说明

[English](RELEASE.md)

GPTgrill-Codexwork 本身有两层，因此 Release 也需要同时发布两类东西：ChatGPT 网页 Skill，以及本地 GPTgrill-Codexwork Runner。
...
| `gptgrill-codexwork-web-skill-vX.Y.Z.zip` | ChatGPT 网页 | 作为一个 Skill 单独上传 |
| `gptgrill-codexwork-local-vX.Y.Z.tgz` | 本地 Mac / Terminal / Codex 环境 | `npm install -g ./gptgrill-codexwork-local-vX.Y.Z.tgz` |
| `gptgrill-codexwork-local-vX.Y.Z.zip` | 本地源码包 | 解压后 `npm install`、`npm run check`、`npm link` |
| `SHA256SUMS.txt` | 校验文件完整性 | `shasum -a 256 -c SHA256SUMS.txt` |
| GitHub 自动生成的 Source code ZIP/TAR | 完整仓库快照 | 开发、审计、备份 |

两个网页 Skill 必须分开发布、分开安装。不要把整个 GPTgrill-Codexwork 仓库 ZIP 当成一个 ChatGPT Skill 上传。

## 本地 Runner 和 Codex App 的关系

GPTgrill-Codexwork 当前真正启动 Codex 的方式是调用本机的 `codex` 命令：

```text
codex exec --full-auto -C <workspace> -
```

所以你平时使用 **Codex App** 没有问题，也不会和 GPTgrill-Codexwork 冲突；但是 GPTgrill-Codexwork Runner 目前并不是去操作 Codex App 的图形界面，而是走 Codex CLI 这条执行路径。
...
如果这两个命令正常，就不用额外处理。你可以继续用 Codex App 做人工交互，同时让 GPTgrill-Codexwork 在后台通过 CLI 跑自动任务。

如果你只有 Codex App，但终端里没有 `codex` 命令，那么还需要额外安装官方 Codex CLI，GPTgrill-Codexwork 才能自动启动任务。

Codex App 和 CLI 属于同一个 Codex 生态；只是 GPTgrill-Codexwork 当前明确依赖 CLI 作为执行入口。
...
然后对每个需要接入 GPTgrill-Codexwork 的项目执行：
