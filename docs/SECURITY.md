...
Goal Loop is intentionally a thin orchestrator, not a new coding sandbox.
...
- Goal Loop refuses to start a goal when the git workspace has pre-existing uncommitted changes.
- Spec/task paths in dispatch manifests must be repository-relative and cannot traverse outside the workspace.
- Runtime state is stored under `~/.goal-loop` with file mode 0600 where supported; it is not committed.
- Goal Loop does not store ChatGPT OAuth tokens, Codex credentials, or C2C secrets.
